...
import (
	...
	"github.com/apexinfosysindia/os-agent/udisks2"
...
func (d datadisk) MarkDataMove() *dbus.Error {
	/* Move request marker for apexos-data.service */
...
func (d datadisk) ChangeDevice(newDevice string) (bool, *dbus.Error) {
	...
	dataDevice, err := udisks2helper.GetRootDeviceFromLabel("apexos-data")
	if err != nil {
	...
	err = udisks2helper.PartitionDeviceWithSinglePartition(newDevice, linuxDataPartitionUUID, "apexos-data-external")
	if err != nil {
		return false, dbus.MakeFailedError(err)
	}
...
const (
	objectPath = "/io/apexos/os/DataDisk"
	ifaceName  = "io.apexos.os.DataDisk"
)
